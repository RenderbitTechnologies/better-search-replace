...
class BSR_AJAX {
	...
	public function process_backup() {
		...
		if ( isset( $tables[$step] ) ) {
			...
			$backup = $db->backup_table( $tables[$step], $page );
			...
			if ( false == $backup['table_complete'] ) {
				...
				if ( isset( $tables[$step] ) ) {
					$message = sprintf( __( 'Backing up table: "%s"...', 'better-search-replace' ), $tables[$step] );
				}
	...
	public function upload_import() {
		...
		if ( isset( $_FILES['bsr_import_file' ] ) && 'sql' == substr( $_FILES['bsr_import_file']['name'], -3 ) ) {

			@unlink( $db->file );

			$tempfile = $_FILES['bsr_import_file']['tmp_name'];
			move_uploaded_file( $tempfile, $db->file );
			$upload_method = 'ajax';
