...
class BSR_DB {
	 ...
	 * @access public
	 */
	public function backup_table( $table, $page ) {
		...
		if ( 0 === $current_page ) {
			...
			if ( $show_create ) {
				$results .= PHP_EOL . "DROP TABLE IF EXISTS `$table`;" . PHP_EOL;
		...
		foreach ( $data as $row ) {
			...
			$queries[] = "INSERT INTO `$table` VALUES " . sprintf( "(%s)", implode( ', ', $vals ) ) . ';' . PHP_EOL;
		}
