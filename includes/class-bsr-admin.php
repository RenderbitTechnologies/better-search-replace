...
class BSR_Admin {
		...
		if ( get_transient( 'bsr_results' ) ) {
			...
			<?php
				...
				if ( isset( $_GET['changes'] ) && isset( $results['table_reports'][$table]['changes'] ) ) {
					...
					printf( '<p style="margin-top:0;"><a href="%s">%s</a></p>', get_admin_url() . 'admin-post.php?action=bsr_view_details', __( '<-- Back to overview', 'better-search-replace' ) );
					...
					foreach ( $results['table_reports'][$table]['changes'] as $change ) {
						...
						if ( true == $results['case_insensitive'] ) {
							$from_str 	= str_ireplace( $results['search_for'], '<span class="bsr-old-val">' . $results['search_for'] . '</span>', $from_str );
						} else {
							$from_str 	= str_replace( $results['search_for'], '<span class="bsr-old-val">' . $results['search_for'] . '</span>', $from_str );
						}
						$to_str 	= str_replace( $results['replace_with'], '<span class="bsr-new-val">' . $results['replace_with'] . '</span>', $to_str );
	...
	public function process_load_profile() {
		...
		if ( isset( $profiles[ $_POST['bsr_profile'] ] ) ) {
			$profile = str_replace( ' ', '+', $_POST['bsr_profile'] );
			$url = get_admin_url() . 'tools.php?page=better-search-replace&bsr_profile=' . esc_attr( $profile );
		} else {
	...
	public function download_backup() {
		...
		header( 'Content-Disposition: attachment; filename="bsr_db_backup.sql"' );
